export function initBoard(){
  ...
  const getPosts=()=>{ try{ return JSON.parse(localStorage.getItem(BOARD_KEY)||'[]'); }catch(e){ return []; } };
  const setPosts=(list)=> localStorage.setItem(BOARD_KEY, JSON.stringify(list));
  const fmtDate=(ts)=>{ const d=new Date(ts); const z=n=>String(n).padStart(2,'0'); return `${d.getFullYear()}-${z(d.getMonth()+1)}-${z(d.getDate())} ${z(d.getHours())}:${z(d.getMinutes())}`; };
