export function initAccidents(){
  const ACC_KEY='accidents_v1';
  const getAcc=()=>{ try{return JSON.parse(localStorage.getItem(ACC_KEY)||'[]')}catch(e){return[]} };
  const setAcc=(v)=> localStorage.setItem(ACC_KEY, JSON.stringify(v));
